...
음절 257자(`.orchestration/notes/t2-subset-risk.md` §1, 재현 명령 포함)만
포함하면 된다. 현재 한글 woff2 3개(300/400/700) 합계 1,525,168 bytes 는 KS X
1001/완성형 한글 11,172음절 전체를 커버하는 서브셋이다. 257자는 그 11,172자의
약 2.3% 이므로, 폰트 서브셋 파일 크기는 글리프 수에 거의 선형 비례하는
`woff2` 압축 특성상 대략 자릿수 단위로 줄어들 여지가 있다 — 다만 이 숫자는
이 태스크에서 실제 서브셋 파일을 만들어 측정하지 않았으므로 예상치이지 확정
수치가 아니다.
...
결정은 사용자에게 남긴다. 위 표에서 눈에 띄는 점: `--font-weight-medium`(500)
은 완전히 죽은 토큰이라 `src/styles/tokens.css:74` 에서 지워도 화면에 영향이
없다(단, 이 문서는 제안만 하며 t2-fonts 는 `src/styles/tokens.css` 를 건드리지
않는다 — t1-visual 소유 파일).

## 확인되지 않은 것 (범위 밖)

- Tauri 실제 webview(WebKit/WebView2)에서의 `.woff` 미로드 여부 — 이 태스크는
  Chromium 계열(Aside 브라우저)에서만 측정했다.
