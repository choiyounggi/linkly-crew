...
| 600 | `--font-weight-semibold` | 아티팩트 패널, 보드 카드 제목, 검색 결과, 레일 라벨, 스레드 헤더 — 6개 선언 지점(5개 파일; `rail.css` 에 2곳: `.rail-card__name`, `.rail-badge`) | 폰트 파일이 없어 이미 700(bold) 얼굴로 렌더되고 있다(캔버스 폭 측정으로 확인, `t2-subset-risk.md` §4) — 700 을 드롭하면 이 6개 지점도 함께 400 으로 밀린다 |
| 700 | `--font-weight-bold`/`--font-weight-heading` | 보드 강조, DAG 노드 강조, 레일 강조, 헤딩류 프리미티브(4곳) + 위 600 사용처 6개 지점(대체 렌더) | 드롭하면 직접 사용처 4곳뿐 아니라 600 을 요청하는 6개 지점도 함께 400 으로 밀려 총 10곳의 시각적 강조가 사라진다 |

**결정(2026-09-02, 사용자):**
- **700 드롭은 하지 않는다** — 직접 사용처 4곳에 더해 600 을 요청하는 6개
  지점까지 총 10곳의 강조가 400 으로 밀리기 때문.
- **`--font-weight-medium`(500)은 제거했다** — 코드 참조 0건(정의 1줄뿐)임을
  재확인하고 `src/styles/tokens.css` 에서 삭제. 폰트 파일을 로드하지 않던
  토큰이라 화면·번들 어느 쪽에도 영향이 없다.

**결정(2026-09-02, t3-font-600): `--font-weight-semibold`(600)도 제거했다.**
`git log -S'semibold'` 와 이 문서를 포함한 `design/` 전체를 검색했으나 "600 을
700 의 별칭으로 의도적으로 쓴다"는 명시적 근거는 없었다 — 위 표는 600 이 700
으로 대체 렌더된다는 **관찰**을 기록했을 뿐, 그 상태를 의도로 채택한다는
결정은 아니었다. CSS font-matching 규칙상(요청한 weight 가 없으면 500-600 은
더 무거운 쪽으로 폴백) 700 요청으로 값만 바꿔도 렌더 결과는 바이트 단위로
동일하므로, `--font-weight-medium`(500) 제거와 같은 방식으로 6개 지점을
`var(--font-weight-bold)` 로 바꾸고 `--font-weight-semibold` 토큰을
`src/styles/tokens.css` 에서 삭제했다. 폰트 파일 구성(9개 woff2)과 빌드 가드는
변경하지 않았다.
